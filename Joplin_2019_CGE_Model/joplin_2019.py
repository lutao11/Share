...
from joplin.Equationlib import *
from joplin.OutputFunctions import *
import os
import pandas as pd
...
modelPath = os.path.join(os.getcwd(), 'joplin')
filePath = os.path.join(modelPath, 'joplin_data')
...
# LABOR SUPPLY
LT = ['L1', 'L2', 'L3', 'L4', 'OUT1', 'OUT2', 'OUT3', 'OUT4']
...
# GOVERNMENTS
G = ['USSOC1', 'USSOC2', 'USSOC3', 'USSOC4', 'PROTAX', 'LOCSTX', 'OTX', 'COUNTYSTX', 'MOSTX', 'MOITX', 'USPIT', 'CYGF', 'FED', 'STATE', 'COUNTY']
...
GX = ['USSOC1', 'USSOC2', 'USSOC3', 'USSOC4', 'PROTAX', 'LOCSTX', 'OTX', 'COUNTYSTX', 'MOSTX', 'MOITX', 'USPIT']

# SALES OR EXCISE TAXES
GS = ['LOCSTX', 'OTX', 'COUNTYSTX', 'MOSTX']
...
# FACTOR TAXES
GF = ['USSOC1', 'USSOC2', 'USSOC3', 'USSOC4', 'PROTAX']
...
GFUS = ['USSOC1', 'USSOC2', 'USSOC3', 'USSOC4']

# EXOGNOUS TRANSFER PMT
GY = ['USSOC1', 'USSOC2', 'USSOC3', 'USSOC4', 'PROTAX', 'LOCSTX', 'OTX', 'COUNTYSTX', 'MOSTX', 'MOITX', 'USPIT', 'CYGF', 'FED', 'STATE', 'COUNTY']

# EXOGNOUS TRANSFER PMT
GTA = ['USSOC1', 'USSOC2', 'USSOC3', 'USSOC4', 'PROTAX', 'LOCSTX', 'OTX', 'COUNTYSTX', 'MOSTX', 'MOITX', 'USPIT', 'CYGF', 'FED', 'STATE', 'COUNTY']
...
# I+G SECTORS
IG = ['AG_MIN', 'UTIL_CONS', 'MANU', 'WHOLE', 'RETAIL', 'TRANS', 'INFO', 'FINANCE', 'REALE', 'PROF_MAN', 'ADMIN_EDU', 'HEALTH', 'ART', 'ACCO', 'OTHER',
      'HS1', 'HS2', 'HS3', 'HS4', 'FED', 'STATE','COUNTY']

# INDUSTRY SECTORS
I = ['AG_MIN', 'UTIL_CONS', 'MANU', 'WHOLE', 'RETAIL', 'TRANS', 'INFO', 'FINANCE', 'REALE', 'PROF_MAN', 'ADMIN_EDU', 'HEALTH', 'ART', 'ACCO', 'OTHER',
     'HS1', 'HS2', 'HS3', 'HS4']

# ENDOGENOUS GOVERNMENTS
IG2 = ['FED', 'STATE', 'COUNTY']

# PRODUCTION SECTORS
IP = ['AG_MIN', 'UTIL_CONS', 'MANU', 'WHOLE', 'RETAIL', 'TRANS', 'INFO', 'FINANCE', 'REALE', 'PROF_MAN', 'ADMIN_EDU', 'HEALTH', 'ART', 'ACCO', 'OTHER']
...
SAM = pd.read_csv(os.path.join(filePath, 'Joplin_SAM_2019.csv'), index_col=0)

# CAPITAL COMP
BB = pd.read_csv(os.path.join(filePath, 'Joplin_CAPCOM_2019.csv'), index_col=0)

# MISC TABLES
miscPath = os.path.join(filePath, 'MiscFile')
JOBCR = pd.read_csv(os.path.join(miscPath, 'Joplin_JOBCORE_2019.csv'), index_col=0)
MISCH = pd.read_csv(os.path.join(miscPath, 'Joplin_HH_2019.csv'), index_col=0)
EMPLOY = pd.read_csv(os.path.join(miscPath, 'Joplin_FACTOR_2019.csv'), index_col=0)
OUTCR = pd.read_csv(os.path.join(miscPath, 'Joplin_OUTCR_2019.csv'), index_col=0)
...
IGTD.loc['STATE', 'MOSTX'] = 1
IGTD.loc['STATE', 'MOITX'] = 1
...
AGFS.loc['L4', G] = SAM.loc['L4', G] + SAM.loc['USSOC4', G]
...
#                              *((SUM(L, RA(L) / RA0(L))/4)/ (CPI(H) / CPI0(H))*(SUM((Z,L), FD(L,Z))/(SUM(H1, HW(H1)* SUM(L, JOBCOR(H1,L)))+ SUM(CM, CMO(CM)) + SUM(L,CMI(L))))+ SUM((CM,L), EXWGEO(CM)/RA(L))/16 *(SUM(CM, CMO(CM))/(SUM(H1, HW(H1)* SUM(L,JOBCOR(H1,L)))+ SUM(CM, CMO(CM)) +SUM(L,CMI(L)))))** (ETARA(H)) 
...
#                              *  ((SUM(GI, PIT0(GI,H)* HH0(H))+ SUM(G, TAUH(G,H)*HH0(H)))/(SUM(GI, PIT(GI,H)* HH(H))+ SUM(G, TAUH(G,H)*HH(H))))**(ETAPIT(H));
  ...
  line2 = (((RA.loc(L) / ExprM(vars, m = RA0.loc[L])).sum(L)/4)/(CPI.loc(H) / ExprM(vars, m = CPI0[H]))\
           * (LSEQ1line2pre.sum(0)/((HW.loc(H1) * ExprM(vars, m = JOBCOR.loc[H1,L].sum(1))).sum(H1) + CMO.loc(CM).sum(CM) + CMI.loc(L).sum(L)))\
           + ((ExprM(vars, m = EXWGEO.loc[CM].sum(0))/RA.loc(L)).sum(0)/16)\
...
# LSEQ2A('OUT1').. CMO('OUT1')=E= CMO0('OUT1')* (((EXWGEO('OUT1') /RA('L1') ))** ECOMO('OUT1'));
  print('LSEQ2A')
...
# LSEQ2B('OUT2').. CMO('OUT2')=E= CMO0('OUT2')* (((EXWGEO('OUT2') /RA('L2') ))** ECOMO('OUT2'));
  print('LSEQ2B')
...
# LSEQ2C('OUT3').. CMO('OUT3')=E= CMO0('OUT3')* (((EXWGEO('OUT3') /RA('L3') ))** ECOMO('OUT3'));
  print('LSEQ2C')
...
# LSEQ2D('OUT4').. CMO('OUT4')=E= CMO0('OUT4')* (((EXWGEO('OUT4') /RA('L4') ))** ECOMO('OUT4'));
  print('LSEQ2D')
...
# LSEQ3(L).. CMI(L)  =E= CMI0(L)* ((( RA(L)/(SUM( H, CPI(H))/5))** ECOMI(L)));
  print('LSEQ3')
  line = ExprM(vars, m = CMI0.loc[L]) * ((RA.loc(L) / (CPI.loc(H).sum(H) / 5))**(ExprM(vars, m = ECOMI.loc[L])))
...
exec(open(os.path.join(modelPath, "joplin_output.py")).read())