  ...
  def get_all_variable_printed(self, output = None):
    ...
    for i in range(len(output)):
      ...
      if len(label) == 1:
        ...
        print(label[0]+'(\''+label[1]+'\',\''+label[2]+'\')'+'='+'{0:.7f}'.format(output[i])+';')
  ...
  def write(self, filename):
    ...
    with open(filename, 'a') as f:
        for i in range(self.nvars):
            ...
            lower = -1e20 if self.LO[i] == None else self.LO[i]
...
class Expr:
  ...
  def __init__(self, item):
    ...
    if type(item) == ExprItem or type(item) == variable or type(item) == int or type(item) == float:
      ...
      self.itemList = [ExprItem(item.itemList[i]) if type(item.itemList[i]) == ExprItem else Expr(item.itemList[i]) for i in range(len(item.itemList))]
      try:
...
class ExprM:
  ...
  def __xor__(self, rhs):
    ...
    if self.info['width'] != 1 or rhs.info['width'] !=1:
      ...
      copy = ExprM(self.vars,em = self)
      copy.m = [[copy.m[i][0] * rhs.m[j][0] for j in range(rhs.info['height'])] for i in range(copy.info['height'])]
  ...
  def loc(self, rows=None, cols = None):
    ...
    copy = ExprM(self.vars,em = self)
    if cols is not None:
      result = [[Expr(copy.m[self.info['rows'].index(i)][self.info['cols'].index(j)]) for j in cols] for i in rows]
      copy.m = result
      ...
      for i in range(self.info['height']):
        for j in range(self.info['width']):
          if operator == None:
            ...
            self.mark[i][j] = mappings[operator](matrix.loc[self.info['rows'][i]][self.info['cols'][j]], value)
  ...
  def write(self, count, filename):
    ...
    for i in range(self.info['height']):
      for j in range(self.info['width']):
        if not self.hasCondition or self.hasCondition and self.mark[i][j]:
          f.write('model.equality'+str(count[0])+' = Constraint(expr='+self.m[i][j].__str__() + ' == 0)\n')
          count[0]+=1
    f.close()
