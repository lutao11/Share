...
# HOUSEHOLDS
H = ['HH1A', 'HH2A', 'HH3A', 'HH4A', 'HH5A', 'HH1B', 'HH2B', 'HH3B', 'HH4B', 'HH5B', 'HH1C', 'HH2C', 'HH3C', 'HH4C', 'HH5C', 
...
# PRODUCTION SECTORS
IP = ['GOODSA', 'TRADEA', 'OTHERA', 'GOODSB', 'TRADEB', 'OTHERB', 'GOODSC', 'TRADEC', 'OTHERC', 'GOODSD', 'TRADED', 'OTHERD', 
      'GOODSE', 'TRADEE', 'OTHERE', 'GOODSF', 'TRADEF', 'OTHERF', 'GOODSG', 'TRADEG', 'OTHERG', 'GOODSH', 'TRADEH', 'OTHERH']

# PRODUCTION GOV.
FG = ['GOODSA', 'TRADEA', 'OTHERA', 'GOODSB', 'TRADEB', 'OTHERB', 'GOODSC', 'TRADEC', 'OTHERC', 'GOODSD', 'TRADED', 'OTHERD', 
      'GOODSE', 'TRADEE', 'OTHERE', 'GOODSF', 'TRADEF', 'OTHERF', 'GOODSG', 'TRADEG', 'OTHERG', 'GOODSH', 'TRADEH', 'OTHERH']
...
'''
TEST1 = pd.Series(index=H, dtype=float).fillna(0.0)
TEST2 = pd.Series(index=H, dtype=float).fillna(0.0)
TEST3 = pd.Series(index=H, dtype=float).fillna(0.0)
TEST4 = pd.Series(index=H, dtype=float).fillna(0.0)
TEST5 = pd.Series(index=H, dtype=float).fillna(0.0)
TEST6 = pd.Series(index=H, dtype=float).fillna(0.0)
TEST7 = pd.Series(index=H, dtype=float).fillna(0.0)
TEST8 = pd.Series(index=H, dtype=float).fillna(0.0)
TEST9 = pd.Series(index=H, dtype=float).fillna(0.0)
TEST10 = pd.Series(index=H, dtype=float).fillna(0.0)
'''
...
#                   * (Y(K) + KPFOR(K)) * ( 1 - SUM(G, TAUFK(G,K) ) ) );
  ...
  print('YEQ(H)')
  line1 = (ExprM(vars, m= A.loc[H,L]) * HW.loc(H) / (ExprM(vars, m= A.loc[H1,L]) * HW.loc(H1)).sum(H1) * (Y.loc(L) * ExprM(vars, m = 1-TAUFL.loc[G,L].sum(0)) - RA.loc(L)*ExprM(vars, m = CMIWAGE.loc[L])*CMI.loc(L)) ).sum(L)
...
# PFEQ(I)..DS(I) =E= DELTA(I)*PROD(F$ALPHA(F,I),(TT(F,I)*FD(F,I))**ALPHA(F,I));
  '''
  print('PFEQ(I)')
  line =  ExprM(vars, m =DELTA.loc[I]) *((ExprM(vars, m = TT.loc[F, I]) * FD.loc(F, I)) ** ExprM(vars, m = ALPHA.loc[F, I])).prod(F)
...
#                         =E= PVA(I) * DS(I) * ALPHA(F,I);
  print('FDEQ(F,I)')
  left = R.loc(F,I) * RA.loc(F) * ExprM(vars, m = 1 + TAUFX_SUM.loc[F,I])* (ExprM(vars, m = TT.loc[F, I]) * FD.loc(F,I))
...
#                              *  ((SUM(GI, PIT0(GI,H)* HH0(H))+ SUM(G, TAUH(G,H)*HH0(H)))/(SUM(GI, PIT(GI,H)* HH(H))+ SUM(G, TAUH(G,H)*HH(H))))**(ETAPIT(H));
  ...
  line4 =( ((ExprM(vars, m = PIT0.loc[GI,H])* ExprM(vars, m = HH0.loc[H])).sum(GI)+ (ExprM(vars, m = TAUH.loc[G,H])*ExprM(vars, m = HH0.loc[H])).sum(G))\
...
#    LSEQ2A('OUTCOM1').. CMO('OUTCOM1')=E= CMO0('OUTCOM1')* (Sum((F11,IG), FD(F11,IG))/sum((F11,IG), FD0(F11,IG)))**(-ECOMO('OUTCOM1')) ;
  print('LSEQ2A')
  line = ExprM(vars, m = CMO0.loc[CM1])*(FD.loc(F11, IG).sum(IG).sum(F11) / ExprM(vars, m = FD0.loc[F11, IG].sum(1).sum(0)))**(ExprM(vars, m = (-1)*ECOMO.loc[CM1]))
...
#    LSEQ2B('OUTCOM2').. CMO('OUTCOM2')=E= CMO0('OUTCOM2')* (Sum((F21,IG), FD(F21,IG))/sum((F21,IG), FD0(F21,IG)))**(-ECOMO('OUTCOM2'));
  print('LSEQ2B')
  line = ExprM(vars, m = CMO0.loc[CM2])*(FD.loc(F21, IG).sum(IG).sum(F21) / ExprM(vars, m = FD0.loc[F21, IG].sum(1).sum(0)))**(ExprM(vars, m = (-1)*ECOMO.loc[CM2]))
...
#    LSEQ2C('OUTCOM3').. CMO('OUTCOM3')=E= CMO0('OUTCOM3')* (sum((F31,IG), FD(F31,IG))/sum((F31,IG), FD0(F31,IG)))** (-ECOMO('OUTCOM3'));
  print('LSEQ2C')
  line = ExprM(vars, m = CMO0.loc[CM3])*(FD.loc(F31, IG).sum(IG).sum(F31) / ExprM(vars, m = FD0.loc[F31, IG].sum(1).sum(0)))**(ExprM(vars, m = (-1)*ECOMO.loc[CM3]))
...
#    LSEQ3(L).. CMI(L)  =E= CMI0(L)* (Sum(FG, FD(L,FG))/Sum(FG, FD0(L,FG)))**(ECOMI(L));
  print('LSEQ3')
  line = ExprM(vars, m = CMI0.loc[L])*((FD.loc(L, FG).sum(FG) / ExprM(vars, m = FD0.loc[L, FG].sum(1)))**(ExprM(vars, m = ECOMI.loc[L])))
...
#                                         *(sum(HSD, CH0(HSD,H))/sum(HSD,CH(HSD,H)))**(1)  ;
  ...
  line5 = ExprM(vars, m = MO0.loc[H]) * (ExprM(vars, m = YD0.loc[H]  / HH0.loc[H]  ) / ( YD.loc(H) / HH.loc(H) ) / ( ExprM(vars, m = CPI0.loc[H]) / CPI.loc(H) ) ) ** ExprM(vars, m = ETAYD.loc[H])
...
############ end shock on specific industries or household groups
'''
