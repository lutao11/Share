# ================================== #
# Galveston 2019 Nonethnic CGE Model #
# ================================== #
...
SAM = pd.read_csv(os.path.join(filePath, 'Galveston_SAM_2019_Nonethnic_HC1.csv'), index_col=0)

# CAPITAL COMP
BB = pd.read_csv(os.path.join(filePath, 'Galveston_CAPCOM_2019_Nonethnic.csv'), index_col=0)

# MISC TABLES
miscPath = os.path.join(filePath, 'MiscFile')
JOBCR = pd.read_csv(os.path.join(miscPath, 'Galveston_JOBCORE_2019_Nonethnic.csv'), index_col=0)
MISCH = pd.read_csv(os.path.join(miscPath, 'Galveston_HH_2019_Nonethnic.csv'), index_col=0)
EMPLOY = pd.read_csv(os.path.join(miscPath, 'Galveston_FACTOR_2019_Nonethnic.csv'), index_col=0)
OUTCR = pd.read_csv(os.path.join(miscPath, 'Galveston_OUTCR_2019_Nonethnic.csv'), index_col=0)
...
# LSEQ3(L).. CMI(L)  =E= CMI0(L)* ((( RA(L)/(SUM( H, CPI(H))/10))** ECOMI(L)));
  print('LSEQ3')
  line = ExprM(vars, m = CMI0.loc[L]) * ((RA.loc(L) / (CPI.loc(H).sum(H) / 10))**(ExprM(vars, m = ECOMI.loc[L])))
