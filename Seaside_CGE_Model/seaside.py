...
from seaside.Equationlib import *
from seaside.OutputFunctions import *
import os
import pandas as pd
...
modelPath = os.path.join(os.getcwd(), 'seaside')
filePath = os.path.join(modelPath, 'seaside')
...
#       / SUM(I, P0(I) * ( 1 + SUM(GS, TAUQ(GS,I) ) ) * CH(I,H) );
  ...
  print('YEQ(H)')
  line1 = (ExprM(vars, m= A.loc[H,L]) * HW.loc(H) / (ExprM(vars, m= A.loc[H1,L]) * HW.loc(H1)).sum(H1) * Y.loc(L) * ExprM(vars, m = 1-TAUFL.loc[G,L].sum(0))).sum(L)
...
#                  *  ((SUM(GI, PIT0(GI,H)* HH0(H))+ SUM(G, TAUH0(G,H)*HH0(H)))/(SUM(GI, PIT(GI,H)* HH(H))+ SUM(G, TAUH(G,H)*HH(H))))**(ETAPIT(H)*1) ;
  ...
  line2 = (((RA.loc(L) / ExprM(vars, m = RA0.loc[L])).sum(L)/5)/(CPI.loc(H) / ExprM(vars, m = CPI0[H]))) ** ExprM(vars, m = ETARA.loc[H])
  ...
  line4 =( ((ExprM(vars, m = PIT.loc[GI,H])* HH.loc(H)).sum(GI)+ (ExprM(vars, m = TAUH0.loc[G,H])* HH.loc(H)).sum(G))\
          / ((ExprM(vars, m = PIT.loc[GI,H])* HH.loc(H)).sum(GI)+ (ExprM(vars, m = TAUH.loc[G,H])*HH.loc(H)).sum(G)) )** ExprM(vars, m = ETAPIT.loc[H])
...
'''
#iNum = 1 # dynamic model itterations
sims = pd.read_csv(os.path.join(filePath, 'Sims Seaside.csv'), index_col=0)
iNum = 1
...
exec(open(os.path.join(modelPath, "seaside_output.py")).read())